	...
	public static class StyleCopRunner
	{
		...
		public static void Execute(ICakeContext context, SettingsDelegate settingsDelegate)
		{
			...
			foreach (var solutionProject in solution.Projects.Where(p => p.Type != FOLDER_PROJECT_TYPE_GUID))
			{
				...
				ProjectParserResult project = null;

				try
				{
					project = projectParser.Parse(solutionProject.Path);
				}
				catch { }


				FilePathCollection projectFiles;
				...
				if (project != null)
				{
					...
					foreach (var projectFile in project.Files)
					{
						...
						projectFiles.Add(projectFile.FilePath);
					}
				}
				else
				{
					projectFiles = GlobbingAliases.GetFiles(context, solutionProject.Path.GetDirectory().ToString() + "/" + "**/*.cs");
				}
				...
				foreach (var projectFile in projectFiles)
				{
					...
					context.Log.Debug($"Stylecop: Found file {projectFile}");
					styleCopConsole.Core.Environment.AddSourceCode(styleCopProject, projectFile.ToString(), null);
				}
